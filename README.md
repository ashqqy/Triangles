...
Если же все $d_{V_i'} = 0$, то треугольники лежат в одной плоскости, и этот случай рассмотрим отдельно позже.
...
Если треугольники являются компланарными, они проецируются на выровненную по осям плоскость, где площади треугольников максимальны. Затем выполняется двумерный тест на пересечение треугольников. Сначала проверяются все ребра $T$​ на пересечение с ребрами $T'$​. Если какое-либо пересечение найдено, то треугольники пересекаются.

В противном случае мы должны проверить, содержится ли $T$​ полностью в $T'$ или наоборот. Это можно сделать, выполнив проверку принадлежности точки треугольнику [Haines94] для одной вершины $T$ относительно $T'$​ и наоборот. -->
